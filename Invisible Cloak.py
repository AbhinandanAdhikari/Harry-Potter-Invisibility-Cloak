# Harry Potter's Invisibility cloak is created by Abhinandan Adhikari
import cv2
import numpy as np

# initialising function for calling of the trackerbars
def hello(x):
    pass; # only for reference
...
while(True):
    ...
    # combining the frames for the required frame
    b = frame[:,:,0];
    g = frame[:,:,1];
    r = frame[:,:,2];
    ...
    # cv2.imshow("Inverse Frame",frame_inv);
    # cv2.imshow("Blanket Area",blanket_area);
